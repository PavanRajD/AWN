...
NS_LOG_COMPONENT_DEFINE("Assignment2_Task1");
...
main(int argc, char* argv[])
{
    ...
    // Set up MAC layer
    WifiMacHelper mac;
    Ssid ssid = Ssid("my-adhoc-network");
    mac.SetType("ns3::AdhocWifiMac", "Ssid", SsidValue(ssid));

    // Install WiFi and MAC on wifiNodes
    NetDeviceContainer devices = wifi.Install(wifiPhy, mac, wifiNodes);
    ...
    // Node 5
    ApplicationContainer clientApps1 = echoClient1.Install(wifiNodes.Get(nWifi - 1));
    clientApps1.Start(Seconds(2.0));
    clientApps1.Stop(Seconds(10.0));

    // Node 4
    ApplicationContainer clientApp2 = echoClient2.Install(wifiNodes.Get(nWifi - 2));
    ...
    wifiPhy.EnablePcap("third", devices.Get(2));
